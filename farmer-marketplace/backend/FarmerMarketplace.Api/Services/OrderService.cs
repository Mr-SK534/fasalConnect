    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<OrderResponseDto> CreateAsync(Guid buyerId, OrderDto dto)
        {
            ...
            foreach (var itemDto in dto.Items)
            {
                ...
                var productQtyInKg = UnitConverter.ToKgQuantity(product.Quantity, product.Unit);
                ...
                if (fromThisFarmer > 0)
                {
                    ...
                    var takenInOriginalUnit = UnitConverter.ToOriginalUnitQuantity(fromThisFarmer, product.Unit);
                ...
                if (remaining > 0)
                {
                    ...
                    var orderedSuppliers = otherSuppliers
                        .OrderByDescending(p => UnitConverter.ToKgQuantity(p.Quantity, p.Unit))
                        .ToList();
                    ...
                    foreach (var supplier in orderedSuppliers)
                    {
                        ...
                        var supplierQtyInKg = UnitConverter.ToKgQuantity(supplier.Quantity, supplier.Unit);
                        var take = Math.Min(remaining, supplierQtyInKg);
                        if (take <= 0) continue;

                        var supplierFarmerPricePerKg = UnitConverter.ToPricePerKg(supplier.Price, supplier.Unit);
                        ...
                        var supplierTakenInOriginalUnit = UnitConverter.ToOriginalUnitQuantity(take, supplier.Unit);
                    ...
                    if (remaining > 0)
                    {
                        var totalAvailableKg = originalProductQuantityKg + orderedSuppliers.Sum(supplier => UnitConverter.ToKgQuantity(supplier.Quantity, supplier.Unit));
